...
public class GeneticAlgorithmScheduler implements IScheduler {
	...
	private static Map<LocalDate, Integer> dayHoursMap;
	private static DBStatus dbStatus;

	public GeneticAlgorithmScheduler(LocalDate month, User generatedBy, Room room, List<UserActive> activeUsers,
			List<UserApplication> userApplications, List<Holyday> holydays, Map<LocalDate, Integer> dayHoursMap,
			ScheduleRepository scheduleRepository, UserDutyRepository userDutyRepository, DBStatus dbStatus) {

		GeneticAlgorithmScheduler.month = month;
		GeneticAlgorithmScheduler.generatedBy = generatedBy;
		GeneticAlgorithmScheduler.room = room;
		...
		GeneticAlgorithmScheduler.dayHoursMap = dayHoursMap;
	...
	public void run() {
		...
		try
		{
			dbStatus.setBusy(room.getId(), month.toString(), true);

			Schedule newSchedule = generateSchedule(month, generatedBy, room, activeUsers, userApplications, holydays, dayHoursMap);
		...
		dbStatus.setBusy(room.getId(), month.toString(), false);
    }
	...
	public Schedule generateSchedule(LocalDate month, User generatedBy, Room room, List<UserActive> activeUsers, List<UserApplication> userApplications, List<Holyday> holydays, Map<LocalDate, Integer> dayHoursMap)
	{
		try
		{
			...
			Schedule result = new Schedule(generatedBy, room, month, LocalDateTime.now(), true);
		...
		catch (Exception e)
		{
			...
			Schedule result = new Schedule(generatedBy, room, month, LocalDateTime.now(), false);
    ...
    /**
     * Returns the hours for a given day, using custom dayHoursMap if available,
     * otherwise falling back to defaults (8 for normal days, 16 for weekends/holidays).
     */
    private static int getHoursForDay(LocalDate day) {
        if (dayHoursMap != null && dayHoursMap.containsKey(day)) {
            return dayHoursMap.get(day);
        }
        if (day.getDayOfWeek() == DayOfWeek.SATURDAY || day.getDayOfWeek() == DayOfWeek.SUNDAY || holydays.stream().anyMatch(h -> h.getDay().equals(day))) {
            return 16;
        }
        return 8;
    }
    ...
    private Set<UserDuty> decodeIndividual(Individual individual, Schedule parentSchedule)
	{
        ...
        for (int i = 0; i < individual.getDefaultGeneLength(); i++) {
        	...
        	if (cnt == 5) {
        		...
        		int dutyHours = getHoursForDay(dutyDay);
